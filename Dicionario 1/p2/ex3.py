...
def calculando(todos,porcentagem):
    if todos[0][1] == todos[1][1]:
      candidatos = (todos[0][1], todos[1][1])
      validaempate(candidatos)
    elif porcentagem > 0.5:
      print(f"O {todos[0][1]} ganhou")
    else:
      if todos[1][1] == todos[2][1]:
        candidatos_ = (todos[1][0], todos[2][0])
        vencedor_empate = validaempate(candidatos_)
      
        verifica = (vencedor_empate, todos[0][0])
        validasegundoturno(verifica)
...
def calcula_voto():
  ...
  if votos == 0:
    ...
    if len(classificacao) == 1:
      ...
      #print(classificacao)
      porcentagem_ = classificacao[0][1] / total_votos

      calculando(classificacao, porcentagem_)
    else:
      print("cabo")
